...
!!$  end subroutine df_field_2d
  ...
  subroutine df_field_2d_staggered(f, deltax, deltay, out_dfdx, out_dfdy)
    ...
    do x = 1, nx - 1 !We go to nx - 1 because we're using a staggered grid
       do y = 1, ny - 1
          ...
          out_dfdx(x,y) = (f(x+1, y) + f(x+1, y+1) - f(x, y) - f(x, y+1))/(2.d0*deltax) 
          !out_dfdy(x,y) = dfdy_2d_stag(f, x, y, deltay)
          out_dfdy(x,y) = (f(x, y+1) + f(x+1, y+1) - f(x,y) - f(x+1, y))/(2.d0*deltay)
       end do
    end do
  ...
  function dfdz_3d_irregular(f, i, j, k, deltas)
    ...
    dfdz_3d_irregular = f(k-1,i,j)*(deltas(k) - deltas(k+1))/((deltas(k) - deltas(k-1))*(deltas(k+1)-deltas(k-1))) + &
         f(k,  i,j)*(deltas(k+1)-2.d0*deltas(k)+deltas(k-1))/((deltas(k)-deltas(k-1))*(deltas(k+1)-deltas(k))) + &
...
!!$  end subroutine d2f_field_stag
  ...
  function d2fdxz_3d(f, i, j, k, delta_x, deltas)
    ...
    d2fdxz_3d = (.5d0/delta_x) * ( &
         (f(k-1, i+1, j) - f(k-1, i-1, j)) * (deltas(k) - deltas(k+1)) / &
         ( (deltas(k) - deltas(k-1)) * (deltas(k+1) - deltas(k-1)) ) + &
         (f(k,   i+1, j) - f(k,   i-1, j)) * (deltas(k+1) + deltas(k-1) - 2.d0*deltas(k)) / &
         ( (deltas(k) - deltas(k-1)) * (deltas(k+1) - deltas(k)) ) + &
         (f(k+1, i+1, j) - f(k+1, i-1, j)) * (deltas(k) - deltas(k-1)) / &
         ( (deltas(k+1) - deltas(k)) * (deltas(k+1) - deltas(k-1)) ) )
  ...
  function d2fdyz_3d(f, i, j, k, delta_y, deltas)
    ...
    d2fdyz_3d = (.5d0/delta_y) * ( &
         (f(k-1, i, j+1) - f(k-1, i, j-1)) * (deltas(k) - deltas(k+1)) / &
         ( (deltas(k) - deltas(k-1)) * (deltas(k+1) - deltas(k-1)) ) + &
         (f(k,   i, j+1) - f(k,   i, j-1)) * (deltas(k+1) + deltas(k-1) - 2.d0*deltas(k)) / &
         ( (deltas(k) - deltas(k-1)) * (deltas(k+1) - deltas(k)) ) + &
         (f(k+1, i, j+1) - f(k+1, i, j-1)) * (deltas(k) - deltas(k-1)) / &
         ( (deltas(k+1) - deltas(k)) * (deltas(k+1) - deltas(k-1)) ) )
