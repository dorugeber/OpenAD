...
#include "xaifBooster/algorithms/Linearization/driver/buildStamp.hpp"
...
int main(int argc,char** argv) { 
  ...
  xaifBoosterLinearization::AlgFactoryManager* theFManager_p=
    dynamic_cast<xaifBoosterLinearization::AlgFactoryManager*>(xaifBoosterLinearization::AlgFactoryManager::instance());
  theFManager_p->init();
  xaifBoosterLinearization::AlgConfig* algConfig_p=theFManager_p->makeAlgConfig(argc,argv,buildStamp);
  try { 
    algConfig_p->config();
  ...
  try {   
    ...
    Cg.genericTraversal(GenericAction::ALGORITHM_ACTION_1); // type change + analyze
