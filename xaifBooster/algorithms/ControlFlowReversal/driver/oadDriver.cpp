...
#include "xaifBooster/algorithms/ControlFlowReversal/driver/buildStamp.hpp"
...
int main(int argc,char** argv) { 
  ...
  xaifBoosterControlFlowReversal::AlgFactoryManager* theFManager_p=
    dynamic_cast<xaifBoosterControlFlowReversal::AlgFactoryManager*>(xaifBoosterControlFlowReversal::AlgFactoryManager::instance());
  theFManager_p->init();
  xaifBoosterControlFlowReversal::AlgConfig* algConfig_p=theFManager_p->makeAlgConfig(argc,argv,buildStamp);
  try { 
    algConfig_p->config();
  ...
  try {   
    ...
    Cg.genericTraversal(GenericAction::ALGORITHM_ACTION_4); // reverse control flow implemented as pass 4
