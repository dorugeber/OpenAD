PROGRAM example
  USE revolve
  IMPLICIT NONE
  INTEGER :: checkpoints,steps, bundle=1, guessedCheckpoints,verbosity=0
  ...
  DO WHILE (.NOT.initialized)
     ...
     READ (*,*) steps
     WRITE (*,FMT='(A)',ADVANCE='NO') 'number of steps to be bundled? :'
     READ (*,*) bundle
     guessedCheckPoints=rvGuess(steps,bundle)
     ...
     WRITE (*,'(A,I8,A,F8.4)') 'revolve suggests :', guessedCheckPoints, ' checkpoints for a factor of ', &
          rvFactor(steps,guessedCheckPoints,bundle)
     WRITE (*,FMT='(A)',ADVANCE='NO') 'how many checkpoints are allowed? :'
     READ (*,*) checkpoints
     initialized=rvInit(steps,checkpoints,errorMsg,bundle=bundle)
