...
c lovcal vars:
      ...
      Real*8  Uphip(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  Vphip(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
c set up for plain execution
         ...
         call EXCH_UV_XY_RS( Uphip,Vphip, withSigns, myThid )
c reset the mode
...
c set up for plain execution
         ...
         our_rev_mode%tape=.FALSE.
         our_rev_mode%adjoint=.FALSE.
         call ADEXCH_UV_XY_RS(Uphip,Vphip,withSignsHelper, myThidHelper)
c reset the mode
         our_rev_mode=our_orig_mode
c copy back
         Uphi%d = Uphip
