...
C$OPENAD XXX Template ad_template.f
      ...
      IF(__value__(X(1)) .GT. 0.0D00) THEN
        ...
        IF(__value__(X(1)) .GT. 1.0D00) THEN
          ...
          CALL setderiv(__deriv__(Y(1)), __deriv__(OpenAD_prp_0))
        ELSE
          IF(__value__(X(1)) .GT. 0.0D00) THEN
            __value__(Y(1)) = __value__(X(1))
            CALL setderiv(__deriv__(Y(1)), __deriv__(X(1)))
      ...
      CALL sax(OpenAD_lin_0, __deriv__(OpenAD_prp_1), __deriv__(Y(1)))
      CALL saxpy(OpenAD_lin_1, __deriv__(OpenAD_prp_2), __deriv__(Y(1))
     > )
      END SUBROUTINE
