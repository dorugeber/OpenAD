/*! \file
  ...
  \version $Id: ManagerICFG.cpp,v 1.1 2007/10/08 19:19:12 utke Exp $
        ...
        std::list<ProcHandle>& worklist)
{
    ...
    if (!prevICFGNode.ptrEqual(0)) {
        icfgEdge = new Edge(icfg, 
                prevICFGNode, callICFGNode, CFLOW_EDGE, CallHandle(0));
        icfg->addEdge(icfgEdge);
    }

    // return node
    OA_ptr<CFG::Node> emptyNode;
    emptyNode = new CFG::Node;
    ...
    OA_ptr<IRCallsiteIterator>
        callsiteIt = mIR->getCallsites(stmt);
    CallHandle call;
    for ( ; callsiteIt->isValid(); ++(*callsiteIt)) {
       call = callsiteIt->current();
    }
    icfgEdge = new Edge(icfg,
                        callICFGNode, prevICFGNode, CALL_RETURN_EDGE, call);
    icfg->addEdge(icfgEdge);
    ...
    OA_ptr<CallGraph::CallGraphInterface> cGraph)
{
  ...
  */
  ...
  while ( ! worklist.empty() ) {
    ...
    for ( ; cfgNodeIter->isValid(); (*cfgNodeIter)++ ) {
        ...
        if (callflag) {
            ...
            OA_ptr<DGraph::NodesIteratorInterface> predIter
                = cfgNode->getSourceNodesIterator();
            for ( ; predIter->isValid(); (*predIter)++ ) {
                OA_ptr<DGraph::NodeInterface> dNode = predIter->current();
                OA_ptr<CFG::Node> predNode = dNode.convert<CFG::Node>();
                // temporarily record
  ...
  std::map<OA_ptr<Node>,
           std::set<OA_ptr<CFG::Node> > >::iterator mapIter;
  for (mapIter=edgeMap.begin(); mapIter!=edgeMap.end(); mapIter++ ) {
      OA_ptr<Node> icfgNode = mapIter->first;
      std::set<OA_ptr<CFG::Node> >& nodeSet = mapIter->second;
      std::set<OA_ptr<CFG::Node> >::iterator nodeIter;
      for (nodeIter=nodeSet.begin(); nodeIter!=nodeSet.end(); nodeIter++) {
          icfgEdge = new Edge(icfg, 
                  mCFGNodeToICFGNode[*nodeIter], 
                  icfgNode, CFLOW_EDGE, CallHandle(0));
          icfg->addEdge(icfgEdge);
      }
  }
