...
CHPF$ TEMPLATE T(514,514) 
CHPF$ PROCESSORS P(4)
      DIMENSION AA(N,N),DD(N,N),X(N,N),Y(N,N),
     1          RX(N,N),RY(N,N),D(N,N)
CHPF$ ALIGN AA(I,J) with T(I,J)
CHPF$ ALIGN DD(I,J) with T(I,J)
CHPF$ ALIGN X(I,J) with T(I,J)
CHPF$ ALIGN Y(I,J) with T(I,J)
CHPF$ ALIGN RX(I,J) with T(I,J)
CHPF$ ALIGN RY(I,J) with T(I,J)
CHPF$ ALIGN D(I,J) with T(I,J)
...
C
  601 LL = LL+1
      WRITE (6,1300)LL,IXCM,JXCM,DXCM,
     1                 IYCM,JYCM,DYCM,
     2                 IRXM,JRXM, RXM,
     3                 IRYM,JRYM, RYM
      ABX  = ABS(RXM)
