...
C  js: 11/16/01 
C
C  fold_operation_() seems to be a hacking only for Linux, see
C  defines.h for MACRO _USE_FOLD_DOT_f which is only defined 
C  on Linux. Not sure why don't they just use AR_xxx functions
C  defined in arith.a?
C
C  One headache is F90 compiler isn't available on Linux. Two
C  intrinsic functions: MODULO and SELECTED_REAL_KIND require
C  F90 compiler. MODULO seems to be easy to replace. For now, 
C  change fold_drive.c such that it always calls AR_selected_
C  real_kind() in arith.a instead of fold_operation_()
C
C  WARNING: AR_selected_real_kind() in arith.a may not be
C  compatible with Linux! Although all other OS's use it by default
C
C  TODO: implement SELECTED_REAL_KIND with F77
C

C js:
C MODULO_INT, MODULO_REAL, FLOOR_REAL are used to implement MODULO
C
	FUNCTION MODULO_INT_8 (x, y)
	integer*8 x, y, a
	real q, xr, yr
	IF ( y .NE. 0 ) THEN		
		xr = x
...
C
C js:
C ERR_SELECTED_REAL_KIND catchs unlikely error when fold_operation_
C is still called with operator SRK_Opr on Linux, after the change 
C to fold_drive.c
C
	SUBROUTINE ERR_SELECTED_REAL_KIND
	call fold_f_abort(129)
	END
	...
	oper_type = oper_type + right_type
      ...
      if (result_type .eq. Real_4) then  
         ...
         elseif (operator .eq. Modulo_Opr) then
            real_4_result = MODULO_REAL_4(real_4_l_opnd, real_4_r_opnd)
      ...
      elseif (result_type .eq. Real_8) then
         ...
         elseif (operator .eq. Modulo_Opr) then
            real_8_result = MODULO_REAL_8(real_8_l_opnd, real_8_r_opnd)
         elseif (operator .eq. Cvrt_Opr) then
            if (left_type .eq. Integer_1) then
      ...
      elseif (result_type .eq. Real_16) then
         ...
         elseif (operator .eq. Modulo_Opr) then
            real_16_result =MODULO_REAL_8(real_16_l_opnd,real_16_r_opnd)
         elseif (operator .eq. Cvrt_Opr) then
            if (left_type .eq. Integer_1) then
     ...
     &                             integer_1_r_opnd)
         elseif (operator .eq. Modulo_Opr) then
            integer_1_result = MODULO_INT_1(integer_1_l_opnd, 
     &                                integer_1_r_opnd)
         elseif (operator .eq. Cvrt_Opr) then
            if (left_type .eq. Integer_1) then
               integer_1_result = integer_1_l_opnd
     ...
     &                             integer_2_r_opnd)
         elseif (operator .eq. Modulo_Opr) then
            integer_2_result = MODULO_INT_2(integer_2_l_opnd, 
     &                                integer_2_r_opnd)
         elseif (operator .eq. Cvrt_Opr) then
            if (left_type .eq. Integer_1) then
     ...
     &                             integer_4_r_opnd)
         elseif (operator .eq. Modulo_Opr) then
            integer_4_result = MODULO_INT_4(integer_4_l_opnd, 
     &                                integer_4_r_opnd)
         elseif (operator .eq. Cvrt_Opr) then
            if (left_type .eq. Integer_1) then
     ...
     &                             integer_8_r_opnd)
         elseif (operator .eq. Modulo_Opr) then
            integer_8_result = MODULO_INT_8(integer_8_l_opnd, 
     &                                integer_8_r_opnd)
         elseif (operator .eq. Cvrt_Opr) then
            if (left_type .eq. Integer_1) then
