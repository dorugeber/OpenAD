...
/* _LINUX_LINUX             Host linux - target_linux  (assume intel for     */
/*                          now) This will change to a more meaningful name  */
/*                          once we figure out what that will be.            */
/*									     */
...
/*									     */
/*   Choose 1:								     */
/*	_MODULE_TO_DOT_o        If file being compiled is called name.f,     */
/*			        then the module information tables will be   */
/*			        sent to the name.o (binary) file via the     */
/*			        interface.  A tmp file name is sent through  */
/*			        the interface.  The backend is required to   */
/*			        put it where it wants it in the .o and then  */
/*			        delete the file. A mechanism has to be       */
/*			        implemented to find the module files again.  */
/*			        This frontend supports Cray loader tables    */
/*			        and SUN ELF files.                           */
...
/*	_MODULE_TO_DOT_M        If file being compiled is called name.f,     */
/*			        then the module information tables will be   */
/*			        sent to name.M directly using fprintf        */
/*			        statements.  When searching for modules,     */
/*			        the compiler will look for .M files.  If a   */
/*			        user specified -p name.o, the file -p name.M */
/*			        will be when resolving USE statements.       */
/*                              If -dm is specified, this is the default.    */
/*                              -em (module to dot mod) overrides this.      */
/*									     */
...
/*                              associated stack variables, move hosted      */
/*                              equiv groups into the host associated stack  */
/*                              and generate a temp at the end of the hosted */
/*                              stack for each child that uses the hosted    */
/*                              stack. False means we assign no offsets to   */
/*                              this block we generate no temp and hosted    */
/*                              stack equiv items are put in their own       */
/*                              stack block, but marked as host associated.  */
...
/*				BUFFER IO, ENDFILE, and REWIND stmts leave   */
/*				the frontend as operators rather than calls. */
...
/*				'start io' operator.                         */
/*	_SEPARATE_DEALLOCATES	Generate seperate calls to DEALLOCATE        */
/*                              routines.                                    */
...
/*	_SM_UNIT_IS_ELEMENT	The unit for stride multipliers is in terms  */
/*                              of elements, rather than words, half words,  */
/*                              bytes                                        */
...
/* Because on source level most things are platform independent add
 * _SOLARIS_SOLARIS here fzhao@rice.edu */
...
/* For compilation on linux.  Also, for now, we piggyback an alpha
 * port upon 64 bit Linux. */

# if defined(_LINUX_LINUX) || defined(__alpha)
#	define _LITTLE_ENDIAN			1 /* for an include file */
...
# if (defined(__alpha))
/* #       undef _HOST_OS_LINUX */
/* #       define _HOST_OS_TRU64                   1 */
# endif
